...
the analysis of the node classification data.

Notes:
- Line 26 requires access to node_classification_tensor.py
...
tensor = np.load('node_classification_tensor.npy')
...
def acc_for_best_configs(dir):
    '''
    Returns a dataframe containing the average accuracy for the best performing
    hyper-parameter configuration for each model on each dataset

    Process:
    - For each model/dataset/config, the val_acc is averaged across trials
    - The config with the highest val_acc is chosen
    - The test_acc for each trial of the best config is averaged
    ...
    for dataset in dataset_indices.keys():
        ...
        df_best_config = df_avg_acc_val_per_config[df_avg_acc_val_per_config['acc_val_mean'] == df_avg_acc_val_per_config.groupby(['model_name'])['acc_val_mean'].transform(max)]
        df_best_config = df_best_config[df_best_config['acc_val_std'] == df_best_config.groupby(['model_name'])['acc_val_std'].transform(max)]
        df_best_config = df_best_config[df_best_config.duplicated(subset = ['model_name']) == False]
        df_best_config['acc_test_mean'] = df_best_config['acc_test_mean'].round(2).astype(str)
        df_best_config['acc_test_std'] = df_best_config['acc_test_std'].round(2).astype(str)
        df_best_config[str(dataset)] = df_best_config['acc_test_mean']

        df_best_config = df_best_config.drop(columns = ['num_layers', 'dimensions', 'learning_rate', 'acc_val_mean', 'acc_test_mean', 'acc_test_std', 'acc_val_std'])
        df_best_config = df_best_config.reset_index(drop = True)
        df_main = df_main.merge(df_best_config, how = 'left')
    return df_main


def acc_for_all_configs(dir):
    '''
    Returns a dataframe containing the average accuracy for each model on each dataset

    Process:
    - For each model/dataset, the test_acc is averaged across all trials of all configs
    ...
    for dataset in dataset_indices.keys():
        ...
        df_across_configs = df_avg_acc_val_per_config.groupby(['model_name']).aggregate({'acc_test_mean': ['mean', np.std]}).reset_index()
        ...
        df_across_configs['acc_test_mean_mean'] = df_across_configs['acc_test_mean_mean'].round(2).astype(str)
        df_across_configs['acc_test_mean_std'] = df_across_configs['acc_test_mean_std'].round(2).astype(str)
        df_across_configs[str(dataset)] = df_across_configs['acc_test_mean_mean']
        df_across_configs = df_across_configs.drop(columns = ['acc_test_mean_mean', 'acc_test_mean_std'])
        df_main = df_main.merge(df_across_configs, how = 'left')
    return df_main


def accuracies_as_heatmap(agg = 'best'):
    '''
    Plots the accuracy dataframe as a heatmap
    ...
    '''
    if agg == 'best':
        df_acc = acc_for_best_configs().set_index('model_name', drop = True).astype(float)

    elif agg == 'all':
        df_acc = acc_for_all_configs().set_index('model_name', drop = True).astype(float)

    else:
        return 'invalid agg entry'

    plt.figure(figsize=(11, 8))
    sns.heatmap(df_acc, annot=True, cmap="Greens", fmt=".2f", linewidths=0.5
    ...
    plt.title('Accuracies for best configurations' if agg == 'best' else 'Accuracies for all configurations')
    plt.show()
...
def stopping_epoch(dataset_idx, model_idx, config, trial):
    ...
    generates the test accuracy at the stopping epoch. Stopping epoch
    is defined as the first instance where val accuracy is maxed. 
    ...
    test_acc_stopping_epoch = sub_tensor[max_index,0]
    return test_acc_stopping_epoch


def acc_across_configs(dataset, model):
    '''
    For a given dataset/model, returns the average test accuracy
    ...
    Returns:
    list: all test accuracies ordered by config id
    '''
    ...
    for config in [x for x in range(36)]:
        ...
        for trial in [x for x in range(10)]:
            acc = stopping_epoch(dataset_idx, model_idx, config, trial)
            trial_accs.append(acc)
        config_acc = np.mean(trial_accs)
        config_accs.append(config_acc)
...
def best_configuration(dataset, model):
    ...
    def val_at_stopping_epoch(dataset_idx, model_idx, config, trial):
        '''
        Returns the val accuracy for a given dataset/model/ 
        config/trial at the stopping epoch. Stopping epoch is
        defined as the first instance where val accuracy is maxed. 
        ...
        Returns:
        float: the val_acc at the stopping epoch
        '''
        sub_tensor = tensor[dataset_idx, model_idx,config,trial]
        max_index = np.argmax(sub_tensor[:,2])
        val_acc_stopping_epoch = sub_tensor[max_index,2]
        return val_acc_stopping_epoch
    
    def val_acc_across_configs(dataset, model):
        '''
        Returns the val accuracy for each config of a given  
        ...
        Returns:
        list: all val accuracies ordered by config id
        '''
        dataset_idx = dataset_indices[dataset]
        model_idx = model_indices[model]
        ...
        for config in [x for x in range(36)]:
            trial_accs = []
            for trial in [x for x in range(10)]:
                acc = val_at_stopping_epoch(dataset_idx, model_idx, config, trial)
                trial_accs.append(acc)
            config_acc = np.mean(trial_accs)
            config_accs.append(config_acc)
    ...
    val_accs = val_acc_across_configs(dataset, model)

    val_acc_arr = np.array(val_accs)
    index_of_max = np.argmax(val_acc_arr)

    return index_of_max

def df_acc_split_by_parameter(hyper_parameter):
    '''
    ...
    Process:
    - For a given model/dataset, calculates the average test_accuracy using
    acc_across_configs (see above definition) as all_accs
    - For each parameter value of the specified hyper-parameter, averages the
    test accuracies as parameter_acc
    - Calculates the percent difference between all_accs and parameter_accs
    ...
    for dataset_category in dataset_categories.keys():
        for dataset in dataset_categories[dataset_category]:
            for model_type in model_categories.keys():
                for model in model_categories[model_type]:
                    all_accs = acc_across_configs(dataset, model)
                    config_acc = np.mean(all_accs)
                    for parameter in hyper_parameter:
                        parameter_accs = [all_accs[i] for i in config_indices[parameter]]
                        parameter_acc = np.mean(parameter_accs)
                        relative_acc = np.round(((parameter_acc/config_acc)*100)-100,2)
                        df.at[(model_type, model), (dataset_category, dataset,parameter)] = relative_acc
...
def best_parameter(hyper_parameter):
    ...
    for dataset in dataset_indices.keys():
        for model in model_indices.keys():
            all_accs = acc_across_configs(dataset, model)
            best_parameter = -1
            best_parameter_acc = 0

            for parameter in hyper_parameter:
                parameter_accs = [all_accs[i] for i in config_indices[parameter]]
                parameter_acc = np.mean(parameter_accs)

                if parameter_acc > best_parameter_acc:
                    best_parameter_acc = parameter_acc
...
def best_hyper_parameter_heatmap(hyper_parameter):
    ...
    if hyper_parameter == layers:
        ...
        df_hyper_parameter.at['GAT', 'PubMed'] = 0
        df_hyper_parameter.at['HGAT', 'PubMed'] = 0
...
def stability_of_best_config(dir):
    ...
    test accuracies across trials

    Process:
    - For each dataset/model/configuration/trial, chooses the 'stopping epoch'
    where val acc is first maximized
    - Then averages the val accs at stopping epochs across all trials of a 
    given dataset/model/configuration
    - Chooses the configuration that has the highest average
    - Once the best config is chosen, collects the test accuracies at the
    ...
    for dataset in dataset_indices.keys():
        ...
        df_best_config['acc_test_mean'] = df_best_config['acc_test_mean'].round(2).astype(str)
        df_best_config['acc_test_std'] = df_best_config['acc_test_std'].round(2).astype(str)
        df_best_config[str(dataset)] = df_best_config['acc_test_std']
...
def stability_of_best_config_heatmap():
    ...
    df_acc = stability_of_best_config().set_index('model_name', drop = True).astype(float)


    plt.figure(figsize=(11, 8))
    sns.heatmap(df_acc, annot=True, cmap="Purples", fmt=".2f", linewidths=0.5
                ,linecolor='black', square=True)
    ...
    plt.title("Stability of acc for best config")
    plt.show()
