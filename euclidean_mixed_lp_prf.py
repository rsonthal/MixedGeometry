...
def eval_test(model, data):
    model.eval()
    prf = {}
    pos_edge_index, neg_edge_index = data['test'].edge_label_index.chunk(2, dim=1)
    link_labels = data['test'].edge_label

    link_logits = model(pos_edge_index, neg_edge_index)
    link_probs = link_logits.sigmoid() # apply sigmoid
    link_preds = link_probs.round()

    prf['precision'] = precision_score(link_labels.cpu(), link_preds.cpu(), average='macro', zero_division=0.0)
    prf['recall'] = recall_score(link_labels.cpu(), link_preds.cpu(), average='macro', zero_division=0.0)
    prf['f1'] = f1_score(link_labels.cpu(), link_preds.cpu(), average='macro', zero_division=0.0)
