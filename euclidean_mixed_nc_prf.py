...
class LorentzLinear(nn.Module):
    ...
    def reset_parameters(self):
        ...
        if self.bias:
            # nn.init.constant_(self.weight.bias, stdv)
...
class HGraphConvLayer(nn.Module):
    def __init__(self, c_in, c_out, node_feats, edge_index):
        ...
        # self.w2 = LorentzLinear(c_in, c_out, bias=False)
...
def export_dict(data_name,model_name, d):
  file_name = f'models_report/{data_name}_{model_name}_report.json'
...
def eval_test(model, data):
  ...
  logits = model()
  mask_test = data['test_mask']
  pred_test = logits[mask_test].max(1)[1].cpu()
  y_test = data.y[mask_test].cpu()

  prec_test = precision_score(y_test, pred_test, average='macro', zero_division=0.0)
  rec_test = recall_score(y_test, pred_test, average='macro', zero_division=0.0)
  f1_test = f1_score(y_test, pred_test, average='macro', zero_division=0.0)
  return prec_test, rec_test, f1_test
...
datasets = {"Cora": data_cora, "CiteSeer": data_citeseer, "PubMed": data_pubmed, \
            "AirportUSA": data_airports_usa, "AirportBrazil": data_airports_brazil,"AirportEurope": data_airports_europe, \
            "WebKB-Cornell": data_cornell, "WebKB-Texas": data_texas, "WebKB-Wisconsin": data_wisconsin}
num_layers = [2, 3, 4]
dims = [32, 64, 128, 256]
    ...
    for num_layer in num_layers:
      for d in dims:
        for lr in learning_rates:
          ...
          rec_config = []
          f1_config = []
          path1 = f'{model_name}/models/'
          filename = f'dataset={data_name}_layers={num_layer}_dims={d}_lr={lr}.pt'
          if os.path.exists(path1+filename):
            file = path1+filename
          else:
            print(f"{model_name} {filename} doesn't exist")
            break
          # print(path)
          model = Net(model_name, d, data, num_layer).to(device)
          # print(file)
          ...
          for model_dict in models_dict:
            ...
            prec, rec, f1 = eval_test(model, data)
            # print(prec, rec, f1)
            prec_config.append(prec)
            rec_config.append(rec)
            f1_config.append(f1)
          prec_model.append(torch.tensor(prec_config))
          rec_model.append(torch.tensor(rec_config))
          f1_model.append(torch.tensor(f1_config))
    prec_data.append(pad_sequence(prec_model, batch_first=True))
    rec_data.append(pad_sequence(rec_model, batch_first=True))
    f1_data.append(pad_sequence(f1_model, batch_first=True))
  precision.append(pad_sequence(prec_data, batch_first=True))
  recall.append(pad_sequence(rec_data, batch_first=True))
  f1_scores.append(pad_sequence(f1_data, batch_first=True))

torch.save(pad_sequence(precision, batch_first=True), 'precision.pt')
torch.save(pad_sequence(recall, batch_first=True), 'recall.pt')
torch.save(pad_sequence(f1_scores, batch_first=True), 'f1_score.pt')

precision = []
recall = []
f1_scores = []
model_names = ['LGCN', 'HGNN', 'HyboNet']
for data_name, data in tqdm(datasets.items()):
  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
  data = data.to(device)
  data.num_classes = int(data.y.max()+1)
    ...
    for num_layer in num_layers:
      for d in dims:
        for lr in learning_rates:
          ...
          f1_config = []
          filename = "dataset=" + data_name + "_layers=" + str(num_layer) + \
              "_dims=" + str(d) + "_lr=" + str(lr)
          path = f"{model_name}/results/prf/{filename}_trial=1.pt"
          if not os.path.exists(path):
            print(f"{path} doesn't exist")
            break
          for trial in range(1,11):
            filename = "dataset=" + data_name + "_layers=" + str(num_layer) + \
              "_dims=" + str(d) + "_lr=" + str(lr) + "_trial=" + str(trial) + ".pt"
            path = f"{model_name}/results/prf/{filename}"
            prf = torch.load(path)
            # print(prec, rec, f1)
            prec_config.append(prf['precision'])
            rec_config.append(prf['recall'])
            f1_config.append(prf['f1'])
          prec_model.append(torch.tensor(prec_config))
          rec_model.append(torch.tensor(rec_config))
          f1_model.append(torch.tensor(f1_config))
    prec_data.append(pad_sequence(prec_model, batch_first=True))
...
torch.save(pad_sequence(precision, batch_first=True), 'precision_hyp.pt')
torch.save(pad_sequence(recall, batch_first=True), 'recall_hyp.pt')
torch.save(pad_sequence(f1_scores, batch_first=True), 'f1_score_hyp.pt')
